...
public:
	...
	GroupImpact checkImpactOfGroupOfInfluence(int agent_position, uint16_t q, uint16_t topology_id)
	{
		std::vector <std::size_t> random_samples = _random_number_handler.getRandomSample(NetworkTopology::NumbersOfNeighbors[topology_id][agent_position], q);
		
		std::size_t sum_of_opinions = 0;
		for(std::size_t sample : random_samples) {
			sum_of_opinions += _list_of_agents[NetworkTopology::Networks[topology_id][agent_position][sample]].getOpinion();
		}

		// if the target agent has less than q neighbors, always all the neighbors are in the group of influence
		std::size_t expected_value = std::min(NetworkTopology::NumbersOfNeighbors[topology_id][agent_position], static_cast<std::size_t>(q));
